  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100] flex flex-col gap-2 pointer-events-none">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`pointer-events-auto px-4 py-3 rounded-lg shadow-lg border backdrop-blur-sm text-sm font-medium flex items-center gap-2 animate-[slideIn_0.3s_ease-out] max-w-sm ${
        ...
        >
          ...
          <span>{t.message}</span>
          <button onClick={() => remove(t.id)} className="ml-auto text-gray-500 hover:text-white shrink-0">
